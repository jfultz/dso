...
EFResidual* EnergyFunctional::insertResidual(PointFrameResidual* r)
{
	...
	connectivityMap[(((uintptr_t)efr->host->frameID) << 32) + ((uintptr_t)efr->target->frameID)][0]++;
...
EFFrame* EnergyFunctional::insertFrame(FrameHessian* fh, CalibHessian* Hcalib)
{
	...
	{
		connectivityMap[(((uintptr_t)eff->frameID) << 32) + ((uintptr_t)fh2->frameID)] = Eigen::Vector2i(0,0);
		if(fh2 != eff)
			connectivityMap[(((uintptr_t)fh2->frameID) << 32) + ((uintptr_t)eff->frameID)] = Eigen::Vector2i(0,0);
	}
...
void EnergyFunctional::dropResidual(EFResidual* r)
{
	...
	connectivityMap[(((uintptr_t)r->host->frameID) << 32) + ((uintptr_t)r->target->frameID)][0]--;
	nResiduals--;
	r->data->efResidual=0;
	delete r;
}
	...
	for(EFFrame* f : frames)
	{
		for(int i=0;i<(int)f->points.size();i++)
		{
			...
			if(p->stateFlag == EFPointStatus::PS_MARGINALIZE)
			{
				...
				for(EFResidual* r : p->residualsAll)
					if(r->isActive())
						connectivityMap[(((uintptr_t)r->host->frameID) << 32) + ((uintptr_t)r->target->frameID)][1]++;
