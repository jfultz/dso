...
void PangolinDSOViewer::publishGraph(const std::map<uintptr_t,Eigen::Vector2i> &connectivity)
{
	...
	for(std::pair<uintptr_t,Eigen::Vector2i> p : connectivity)
	{
		...
		uintptr_t inverseKey = (((uintptr_t)target) << 32) + ((uintptr_t)host);
